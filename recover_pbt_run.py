...
# Original run directory to recover
RECOVERY_RUN_DIR = Path("/mnt/g/To Process/PMA17/PMA17 2020-10-22 Session-1/Analysis/LFADS/runs/lfads-torch-2afc/2afc/250722_161929_2afc_single")
...
PROJECT_STR = "lfads-torch-2afc"
DATASET_STR = "2afc"
# ------------------------------

# Check if recovery directory exists
if not RECOVERY_RUN_DIR.exists():
    raise FileNotFoundError(f"Recovery directory not found: {RECOVERY_RUN_DIR}")

print(f"Recovering run from: {RECOVERY_RUN_DIR}")
...
# Set the mandatory config overrides (same as original)
mandatory_overrides = {
...
print("Starting recovery of PBT run...")

# Resume the hyperparameter search from the existing directory
analysis = tune.run(
    ...
    metric=metric,
    mode="min",
    name=RECOVERY_RUN_DIR.name,
    resume=True,  # This is the key parameter for recovery
    ...
    config=mandatory_overrides,  # Note: Don't need init_space for resume
    resources_per_trial=dict(cpu=2, gpu=0.33),
    num_samples=num_trials,
    local_dir=RECOVERY_RUN_DIR.parent,
...
best_model_dir = RECOVERY_RUN_DIR / "best_model"
